...
/* Run this code with the child process seg faulting to see what
    read() returns in the parent process */
...
int main()
{
  ...
  /* fd table:     [each process has its own fd table]
     ...
     *** after we call close() in child and parent below ***
     ...
     if the child process crashes before it writes any data:
     [PARENT]
     0: stdin
     1: stdout
     2: stderr                   +--------+
